...
func TestMiddleware(t *testing.T) {
	...
	// match 2
	app = fiber.New()
	app.Use(Middleware(rkmidcors.WithAllowOrigins("http://do-not-pass-through")))
	app.Get("/ut-path", func(ctx *fiber.Ctx) error {
		return nil
	})
	req = httptest.NewRequest(http.MethodGet, "/ut-path", nil)
	...
	// match 3.1
	app = fiber.New()
	app.Use(Middleware(rkmidcors.WithAllowCredentials(true)))
	app.Get("/ut-path", func(ctx *fiber.Ctx) error {
		return nil
	})
	req = httptest.NewRequest(http.MethodGet, "/ut-path", nil)
	...
	// match 4.2
	app = fiber.New()
	app.Use(Middleware(rkmidcors.WithAllowHeaders("ut-header")))
	app.Options("/ut-path", func(ctx *fiber.Ctx) error {
		return nil
	})
	req = httptest.NewRequest(http.MethodOptions, "/ut-path", nil)
	...
	// match 4.3
	app = fiber.New()
	app.Use(Middleware(rkmidcors.WithMaxAge(1)))
	app.Options("/ut-path", func(ctx *fiber.Ctx) error {
		return nil
	})
	req = httptest.NewRequest(http.MethodOptions, "/ut-path", nil)
