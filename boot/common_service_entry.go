...
const (
	...
	CommonServiceEntryDescription = "Internal RK entry which implements commonly used API with fiber framework."
)
...
// @contact.name rk-dev
// @contact.url https://github.com/rookie-ninja/rk-fiber
// @contact.email lark@pointgoal.io

// @license.name Apache 2.0 License
// @license.url https://github.com/rookie-ninja/rk-fiber/blob/master/LICENSE.txt
...
// 5: Apis Returns list of apis registered in fiber router
...
func doHealthy(*fiber.Ctx) *rkentry.HealthyResponse {
...
func (entry *CommonServiceEntry) Healthy(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}

	ctx.Context().SetStatusCode(http.StatusOK)
	return ctx.JSON(doHealthy(ctx))
}

// Helper function of /gc
func doGc(*fiber.Ctx) *rkentry.GcResponse {
...
// @Router /rk/v1/gc [get]
func (entry *CommonServiceEntry) Gc(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}

	ctx.Context().SetStatusCode(http.StatusOK)
	return ctx.JSON(doGc(ctx))
}

// Helper function of /info
func doInfo(*fiber.Ctx) *rkentry.ProcessInfo {
...
func (entry *CommonServiceEntry) Info(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}

	ctx.Context().SetStatusCode(http.StatusOK)
	return ctx.JSON(doInfo(ctx))
}

// Helper function of /configs
func doConfigs(*fiber.Ctx) *rkentry.ConfigsResponse {
...
func (entry *CommonServiceEntry) Configs(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}

	ctx.Context().SetStatusCode(http.StatusOK)
	return ctx.JSON(doConfigs(ctx))
}

// Construct swagger URL based on IP and scheme
func constructSwUrl(entry *FiberEntry, ctx *fiber.Ctx) string {
...
func doApis(ctx *fiber.Ctx) *rkentry.ApisResponse {
	res := &rkentry.ApisResponse{
		Entries: make([]*rkentry.ApisResponse_Entry, 0),
	}

	fiberEntry := getEntry(ctx)

	if fiberEntry != nil {
		routes := fiberEntry.ListRoutes()
		for j := range routes {
			info := routes[j]
			...
			entry := &rkentry.ApisResponse_Entry{
				Rest: &rkentry.ApisResponse_Rest{
					...
					Pattern: info.Path,
					Method:  info.Method,
					SwUrl:   constructSwUrl(fiberEntry, ctx),
				},
				EntryName: fiberEntry.GetName(),
			}
...
func (entry *CommonServiceEntry) Apis(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}
	...
	ctx.Context().SetStatusCode(http.StatusOK)
	return ctx.JSON(doApis(ctx))
}

// Helper function of /sys
func doSys(*fiber.Ctx) *rkentry.SysResponse {
...
func (entry *CommonServiceEntry) Sys(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}

	ctx.Context().SetStatusCode(http.StatusOK)
	return ctx.JSON(doSys(ctx))
}
...
func doReq(ctx *fiber.Ctx) *rkentry.ReqResponse {
	metricsSet := rkfibermetrics.GetServerMetricsSet(ctx)
	if metricsSet == nil {
	...
	vector := metricsSet.GetSummary(rkfibermetrics.ElapsedNano)
	if vector == nil {
		return &rkentry.ReqResponse{
			Metrics: make([]*rkentry.ReqMetricsRK, 0),
		}
	...
	fiberEntry := GetFiberEntry(rkfiberctx.GetEntryName(ctx))
	if fiberEntry != nil {
		routes := fiberEntry.ListRoutes()
		for j := range routes {
			info := routes[j]
...
func (entry *CommonServiceEntry) Req(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}

	ctx.Context().SetStatusCode(http.StatusOK)
	return ctx.JSON(doReq(ctx))
}
...
// Helper function of /entries
func doEntries(ctx *fiber.Ctx) *rkentry.EntriesResponse {
...
func (entry *CommonServiceEntry) Entries(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}

	ctx.Context().SetStatusCode(http.StatusOK)
	return ctx.JSON(doEntries(ctx))
}

// Helper function of /entries
func doCerts(ctx *fiber.Ctx) *rkentry.CertsResponse {
...
func (entry *CommonServiceEntry) Certs(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}

	ctx.Context().SetStatusCode(http.StatusOK)
	return ctx.JSON(doCerts(ctx))
}
...
// Helper function of /logs
func doLogs(ctx *fiber.Ctx) *rkentry.LogsResponse {
...
func (entry *CommonServiceEntry) Logs(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}

	ctx.Context().SetStatusCode(http.StatusOK)
	return ctx.JSON(doLogs(ctx))
}

// Extract fiber entry from zap middleware
func getEntry(ctx *fiber.Ctx) *FiberEntry {
	if ctx == nil {
		return nil
	}

	entryRaw := rkentry.GlobalAppCtx.GetEntry(rkfiberctx.GetEntryName(ctx))
	if entryRaw == nil {
		return nil
	}
...
func (entry *CommonServiceEntry) Deps(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}
...
// Extract fiber entry from zap middleware
func doDeps(ctx *fiber.Ctx) *rkentry.DepResponse {
...
func (entry *CommonServiceEntry) License(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}

	ctx.JSON(doLicense(ctx))
	return nil
}

// Extract fiber entry from zap middleware
func doLicense(ctx *fiber.Ctx) *rkentry.LicenseResponse {
...
func (entry *CommonServiceEntry) Readme(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}

	ctx.JSON(doReadme(ctx))
	return nil
}

// Extract fiber entry from zap middleware
func doReadme(ctx *fiber.Ctx) *rkentry.ReadmeResponse {
...
func (entry *CommonServiceEntry) Git(ctx *fiber.Ctx) error {
	if ctx == nil {
		return internalError.Err
	}
...
// Extract fiber entry from zap middleware
func doGit(ctx *fiber.Ctx) *rkentry.GitResponse {
