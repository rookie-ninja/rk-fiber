...
const (
	...
	StaticFileHandlerEntryDescription = "Internal RK entry which implements static file handler fiber framework."
)
...
func (entry *StaticFileHandlerEntry) GetFileHandler() fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		// Trim prefix with path user defined in order to get file path
		p := strings.TrimSuffix(strings.TrimPrefix(ctx.Path(), entry.Path), "/")
		if len(p) < 1 {
			p = "/"
		}
		p = path.Join("/", p)
