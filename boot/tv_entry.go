...
const (
	...
	TvEntryDescription = "Internal RK entry which implements tv web with fiber framework."
)
...
func readFileFromPkger(filePath string) []byte {
	...
	if file, err = pkger.Open(path.Join("github.com/rookie-ninja/rk-fiber:/boot", filePath)); err != nil {
		return []byte{}
	}
...
func (entry *TvEntry) AssetsFileHandler() fiber.Handler {
...
// @Router /rk/v1/tv/ [get]
func (entry *TvEntry) TV(ctx *fiber.Ctx) error {
	if ctx == nil {
		return fmt.Errorf("nil fiber context")
	}
