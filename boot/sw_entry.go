...
const (
	...
	SwEntryDescription = "Internal RK entry which implements swagger with fiber framework."
)
...
func (entry *SwEntry) AssetsFileHandler() fiber.Handler {
...
func (entry *SwEntry) ConfigFileHandler() fiber.Handler {
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	//	case "/sw/swagger-config.json":
	//		ctx.Response().SetBodyStream(strings.NewReader(swConfigFileContents), -1)
	//	default:
	//		p = strings.TrimPrefix(p, "/sw/")
	//		value, ok := swaggerJsonFiles[p]
	//
	//		if ok {
	//			ctx.Context().SetContentType("text/html; charset=utf-8")
	//			ctx.Response().SetBodyStream(strings.NewReader(value), -1)
	//			break
	//		}
	//
