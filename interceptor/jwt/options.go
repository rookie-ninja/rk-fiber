...
var (
	...
	defaultSkipper = func(ctx *fiber.Ctx) bool {
		return false
	}
...
type ParseTokenFunc func(auth string, ctx *fiber.Ctx) (*jwt.Token, error)
...
func newOptionSet(opts ...Option) *optionSet {
	...
	if reflect.ValueOf(set.Skipper).Pointer() == reflect.ValueOf(defaultSkipper).Pointer() {
		set.Skipper = func(ctx *fiber.Ctx) bool {
...
type optionSet struct {
	...
	ParseTokenFunc func(auth string, c *fiber.Ctx) (*jwt.Token, error)
}
...
func (set *optionSet) defaultParseToken(auth string, ctx *fiber.Ctx) (*jwt.Token, error) {
	token := new(jwt.Token)
	var err error
...
func jwtFromHeader(header string, authScheme string) jwtExtractor {
	return func(ctx *fiber.Ctx) (string, error) {
		auth := string(ctx.Request().Header.Peek(header))
		l := len(authScheme)
