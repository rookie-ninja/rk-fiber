...
func TestNewOptionSet(t *testing.T) {
	// With empty option
	set := newOptionSet()
	assert.Equal(t, rkfiberinter.RpcEntryNameValue, set.EntryName)
	assert.Equal(t, rkfiberinter.RpcEntryTypeValue, set.EntryType)
	...
	set = newOptionSet(
		...
		WithSkipper(func(ctx *fiber.Ctx) bool {
			return true
		}),
