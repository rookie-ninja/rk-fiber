...
type Limiter func(ctx *fiber.Ctx) error

// NoopLimiter will do nothing
type NoopLimiter struct{}

// Limit will do nothing
func (l *NoopLimiter) Limit(*fiber.Ctx) error {
	return nil
}
...
func (l *ZeroRateLimiter) Limit(*fiber.Ctx) error {
...
func (l *tokenBucketLimiter) Limit(ctx *fiber.Ctx) error {
	l.delegator.Wait(1)
	return nil
}
...
func (l *leakyBucketLimiter) Limit(ctx *fiber.Ctx) error {
	l.delegator.Take()
	return nil
}
...
func (set *optionSet) Wait(ctx *fiber.Ctx) (time.Duration, error) {
	now := time.Now()

	limiter := set.getLimiter(ctx.Path())
