...
type Skipper func(ctx *fiber.Ctx) bool

// CsrfTokenExtractor defines a function that takes `echo.Context` and returns
// either a token or an error.
type csrfTokenExtractor func(*fiber.Ctx) (string, error)
...
func csrfTokenFromHeader(header string) csrfTokenExtractor {
	return func(ctx *fiber.Ctx) (string, error) {
		token := string(ctx.Request().Header.Peek(header))
		if token == "" {
...
func csrfTokenFromQuery(param string) csrfTokenExtractor {
	return func(ctx *fiber.Ctx) (string, error) {
		token := string(ctx.Context().QueryArgs().Peek(param))
		if token == "" {
